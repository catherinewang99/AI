...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
        	...
        	for state in self.mdp.getStates():
        		...
        		for a in self.mdp.getPossibleActions(state):
        			val = max(self.computeQValueFromValues(state, a), val)
        			counter[state] = val
        	self.values = counter
    ...
    def computeQValueFromValues(self, state, action):
        ...
        g = self.discount
        val = 0
        for next_state, p in tsas:
        	val += p*(self.mdp.getReward(state, action, next_state) + (g * self.values[next_state]))
        return val
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
    	...
    	for i in range(self.iterations):
        	...
        	for p in pred[state]:
        		pred_q = [self.computeQValueFromValues(p, action) for action in self.mdp.getPossibleActions(p)]
        		val = abs(max(pred_q) - self.values[p])
        		if val > self.theta:
        			prioq.update(p, -val)
